use safa_abi::{
    ...
    errors::ErrorStatus,
    sockets::{SockBindAbstractAddr, SockBindAddr, SockCreateKind, SockDomain, SockMsgFlags},
};
...
impl UnixSockConnectionBuilder {
    ...
    pub fn connect(self) -> Result<UnixSockConnection, ErrorStatus> {
        ...
        if self.kind == UnixSockKind::SeqPacket {
...
impl UnixSockConnection {
    ...
    pub fn peek(&mut self, buf: &mut [u8]) -> Result<usize, ErrorStatus> {
        syscalls::sockets::recv_from(self.inner_ri, buf, SockMsgFlags::PEEK, None)
    }
